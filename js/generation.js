...
let tasks = [
	[
		[
			"Физика звезд #1",
			"Определите на какую длину волны приходится максимум излучения звезды с температурой first_param K.",
			[10000, 5],
			[20, 8],
			"0.29/first_param * 10000000",
			"Воспользуйтесь законом Вина : λ = 0.29 / T [см]."

		],
		[
			"Физика звезд #2",
			"Две звезды имеют одинаковые угловые диаметры, расстояние до них неизвестно. Их температуры различаются в first_param раза. Найдите разницу болометрических звёздных величин звезд.",
			[2.5, 4],
			[10, 20],
			"Math.abs(-2.5*Math.log10(Math.pow(first_param, 4)))",
			"Найдите отношение светимостей, далее посчитайте разность масс как -2.5lg(L1/L2)."

		],
		[
			"Физика звезд #3",
			"Видимая звёздная величина звезды first_paramm, её светимость second_param L⊙. Найдите расстояние до звезды.",
			[-0.5, 5],
			[0.5, 5],
			"Math.pow(10,  (first_param+5-4.8+2.5*Math.log10(second_param))/5)",
			"Сравните звезду с солнцем в абсолютной звездной величине."

		],
		[
			"Физика звезд #4",
			"Звезда имеет видимую звездную величину first_paramm и параллакс 0.3, а также температуру поверхности second_param К. Найдите её светимость.",
			[0.5, 5],
			[6500, 5],
			"Math.pow(10, -0.4*(first_param+5+5*Math.log10(0.3) - 4.8) )",
			"Найдите абсолютную звездную величину, а потом отношение светимостей звезды и солнца."

		],
		[
			"Физика звезд #5",
			"Звезда имеет светимость first_param L⊙ и температуру поверхности second_param К. Найдите её радиус.",
			[5, 5],
			[6500, 5],
			"Math.sqrt(first_param * Math.pow((5800/second_param), 4))",
			"Найдите абсолютную звездную величину, а потом отношение светимостей звезды и солнца."

		],
		[
			"Физика звезд #6",
			"Абсолютные звёздные величины звёзд first_paramm и second_paramm , температуры 4300 K и 6800 K соответственно. Найдите отношение плотности первой звезды к плотности второй, если их массы равны.",
			[1, 5],
			[2.5, 5],
			"Math.pow(Math.pow(10, -0.4 * (first_param-second_param)) * Math.pow(6800/4300, 4)   , -3/2)",
			"Отношение плотностей зависит только от радиусов звезд, так как их массы равны, осталось найти радиусы."

		],
		[
			"Физика звезд #7",
			"Максимум длины излучения звезды приходится на first_param Å. Определите температуру её поверхности.",
			[7000, 4],
			[2.5, 5],
			"2.9*10000000/first_param",
			"Воспользуйтесь законом Вина : λ = 0.29 / T [см]."
		...
		[
			"Физика звезд #8",
			"Максимум длины излучения звезды приходится на first_param Å, а её светимость равна second_param L⊙ . Определите её радиус.",
			[7000, 4],
			[2.5, 5],
			"Math.sqrt(1/(Math.pow(2.9*10000000/first_param /5800, 4) / second_param))",
			"Воспользуйтесь законом Вина : λ = 0.29 / T [см]. Далее воспользуйтесь формулой Стефана-Больцмана."

		],
		
		[
			"Физика звезд #9",
			"Максимум длины излучения звезды приходится на first_param Å. Определите на какой длине волны будет максимум излучения, если светимость будет той же, а радиус уменьшится в second_param раза.",
			[7000, 4],
			[3, 5],
			"2.9/(2.9*10000000/first_param * Math.sqrt(second_param)) * Math.pow(10, 7)",
			"Воспользуйтесь формулой Стефана-Больцмана. Воспользуйтесь законом Вина : λ = 0.29 / T [см]. "

		],
		
		[
			"Физика звезд #10",
			"Абсолютная звёздная величина звезды first_paramm. Оцените её массу, если она находится на главной последовательности на диаграмме Герцшпрунга-Рассела.",
			[4, 4],
			[3, 5],
			"Math.pow(Math.pow(10, -0.4*(first_param-4.8)), 1/4)",
			"Так как звезда лежит на главной последовательности L1/L2 = (M1/M2)^4. Осталось найти L1/L2. "
...
hidden.addEventListener("click", function (event) {
	...
	if (event.target.getAttribute("submit") == "submit") {
		...
		let answer = round_to_dec(par.getAttribute("ans"))
